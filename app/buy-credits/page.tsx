...
function BuyCredits() {
  ...
  return (
    <div className='min-h-screen p-10 md:px-20 lg:px-40 text-center bg-[#cad3ff]'>
        <h2 className='text-4xl font-bold text-[#4a5c99]'>Add More Credits</h2>
        <div className='grid grid-cols-1 md:grid-cols-2 mt-10 gap-10 items-center justify-center'>
            <div>
                {Options.map((option,index)=>(
                    <div key={index} className={`p-6 my-3 border bg-[#4a5c99] text-center rounded-lg text-white cursor-pointer
                     hover:scale-105 transition-all ${selectedOption==option.id && 'bg-black'}`}onClick={()=>setSelectedOption(option.id)}>
            ...
            <div>
                ...
                onCancel={()=>notifyError('Payment cancel')}
